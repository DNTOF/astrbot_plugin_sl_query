...
- 数据按群聊/私聊会话隔离
- 支持绑定、解绑、列表、查询等完整命令
- Exiled服务端插件：[SLDataAPI](https://github.com/DNTOF/SLDataAPI)
### 命令列表
...
| `/sllist` | 显示本会话所有绑定服务器 |
| `/unbind <服务器名>` | 解绑指定服务器 (暂仅支持官方API绑定的服务器) |
| `/unbindall` | 解绑本会话所有服务器 |

## 如何绑定
1. 用 /bind 绑定（id 必须是 !id 返回的Account ID）
2. 或使用/bindex 绑定Exiled服务端（ip为游戏服务器ip，key为服务端插件配置文件填写的key）
